...
| **QEMU differential rig** (`qemu-rig.sh` + `qemu-mon.py`) | Boots the 9.0.1 ROM on QEMU mac99 + 9.2.1 CD as a behavioral oracle — "what does the NK/Mac OS expect here?" answered by observation instead of RE. Auto-rebuilds the test ISO from persistent assets if missing. Per-run directory contains monitor socket, optional device trace, and auto-generated probe summary (Ticks, interrupt vectors, 68k low-mem flags, handler disassembly). Use `qemu-mon.py --disasm 0xADDR` to disassemble from live guest virtual memory. **Oracle is valid from NK entry onward (OpenBIOS ≠ Apple OF). Device MMIO addresses differ from real hardware — use for behavior, not addresses.** | `SheepShaver/tools/qemu-rig.sh --help`; `docs/planning/machine/VIA-IFR-RECON.md` §5; pitfalls: `LEARNINGS.md` "QEMU rig bringup". |
