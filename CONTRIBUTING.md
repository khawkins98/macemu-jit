...
| **QEMU differential rig** (`qemu-rig.sh` + `qemu-mon.py`) | Boots the 9.0.1 ROM on QEMU mac99 + 9.2.1 CD as a behavioral oracle — "what does the NK/Mac OS expect here?" answered by observation instead of RE. Produces monitor socket + probe file; `--ladder 3,5,10,30` (default) probes key addresses at multiple boot stages so value evolution is visible (early ROM init → Mac OS handoff). **Stage-matching rule:** compare the 3–5s bracket against our early-boot SS probes; don't compare against the 50s (Finder) values — the level-1 vector and Hnfo fields change through the boot. **Oracle is valid from NK entry onward (OpenBIOS ≠ Apple OF). Device MMIO addresses differ from real hardware — use for behavior, not addresses.** | `SheepShaver/tools/qemu-rig.sh --help`; `docs/planning/machine/VIA-IFR-RECON.md` §5; pitfalls + stage-matching: `LEARNINGS.md` "VIA-IFR session 2". |
| **SS_PROBE_68K** (68k handler probing) | Fires at the DR dispatch hook when the 68k PC matches the target — shows the 68k regfile (d0–d7, a0–a7) and PPC context at handler entry. **Use for 68k code paths; SS_PROBE_PC is probe-blind to 68k addresses.** Syntax: `SS_PROBE_68K=0xPC:N` (first N matches, linear, edge-triggered). | `docs/AGENT-CONTEXT.md` Instruments section; `LEARNINGS.md` "VIA-IFR session 2 §1". |
