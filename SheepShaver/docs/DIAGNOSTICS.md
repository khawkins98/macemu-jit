...
| `[WATCH-SAMPLE addr=... value=... obs=N ...]` | `SS_JIT_WATCH_ADDR` at logarithmic record counts | edge-independent value sample per watched word (1, 10, 100, … records) — NOT a change report |
...
| `SS_JIT_WATCH_ADDR=hex[:lenhex],…` | Guest-memory watchpoints (**HEX, no `0x` prefix**, comma-separated, up to **8 word slots** — raised from 4 in 2026-06-12; per-record cost is one read+compare per slot, trivial next to the ring record it rides on — 4-byte aligned). **Requires `SS_JIT_TRACE_RING=1`** — the check lives in `jit_ring_record()`, so without the ring nothing fires. Emits `[WATCH] pc=PPPPPPPP addr=AAAAAAAA value=VVVVVVVV  (was WWWWWWWW ...)` on each detected change. PC is block-entry granularity for JIT, exact instruction for interpreter. Grep for `[WATCH]` to parse programmatically. (Doc fix 2026-06-11: the parser is `strtoul(tok, NULL, 16)` — earlier docs said decimal.) **Span form** (2026-06-12): `ADDR:LEN` (LEN = bytes, hex, capped `0x10`) expands to one watch slot per word — `168:8` watches 0x168 AND 0x16c (the Ticks long: its LSB lives in 0x16c, the word that was historically watch-blind). Each span word consumes a slot; the expansion is announced (`[WATCH] span 168:8 -> 2 word slot(s)`), overflow words are dropped with a note. **Periodic sampling** (2026-06-12): each watched word also emits `[WATCH-SAMPLE addr=… value=… obs=N record=#N pc=…]` at logarithmic record counts (obs = 1, 10, 100, …), independent of change edges — frozen-vs-moving is provable without a change edge, and a zero-over-zero regime is at least visible as a stable sample. **Honest limitation**: this remains a SAMPLING CHANGE-DETECTOR read once per ring record — value-identical stores (zero-over-zero) never trip the change line, and a change that reverts within one record window is invisible. True store-event detection would need store instrumentation in JIT codegen — out of scope. |
...
| Env var | Effect |
|---|---|
| `SS_DR_R24_RING=1` | 68k-PC transition ring: records guest **r24** (the DR emulator's 68k PC) at every JIT dispatcher block entry into a 2M-entry ring, deduped against the last 4 recorded values (loop ping-pong suppressed). Since the instrument batch, suppression no longer masks recurrence: each suppressed repeat increments the matched slot's count, and the dump prints `PC*N` for slots seen more than once (`N` = total observations = 1 + suppressed repeats, covering both consecutive same-r24 dispatches and last-4 loop recurrence — the Q-F4 confound is closed; plain `PC` records are unchanged). Dumped as a `[R24RING] N transitions recorded …` block to stderr at exit, **and now also on SIGSEGV** (rewired 2026-06-12, instr-hardening item 3: the SIGSEGV handler flushes the r24 ring DIRECTLY and EARLY — before the re-fault-prone register/disasm dumps — and mid-run trace-ring dumps from watch/stall/trigger no longer consume the crash once-shot; both were recorded causes of "no r24 dump on SIGSEGV". **Honest trigger conditions**: the crash flush fires only on signals the sigsegv library routes (SIGSEGV/SIGBUS) — a SIGTRAP death still produces no dump; clean/SIGTERM exits dump via atexit as before, and a handled crash that reaches exit may print the dump twice — harmless). Pair with `SS_TERM_DUMP=1` for timeout-killed boots. **This is the only way to observe 68k control flow** — 68k PCs are PPC-probe-blind (`SS_PROBE_PC` keys on PPC block-entry PCs); for a register-file dump at a 68k PC, see `SS_PROBE_68K` below. |
