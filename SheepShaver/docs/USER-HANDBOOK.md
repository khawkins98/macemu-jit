...
| `SS_SEED_MEM=…` | No-recompile guest-memory poke. Immediate form `0xADDR=0xVAL` (applied at NW-trampoline-end) or PC-triggered `0xPC:0xADDR=0xVAL` (at first JIT visit of `0xPC`); up to 16 semicolon-separated 32-bit writes. MMIO ranges refused. See `DIAGNOSTICS.md` env-var table and CLAUDE.md "Guest Memory Seeding". |
