...
# --- Same ev_mixed element-order class: byte/halfword/word MERGES and the PACK.
# Self-operand with distinct lane bytes (v1 = 00..0F) so the result word depends on
# which elements get selected — exposes the JIT's wrong element indexing. Keep only
# those `make test-jit` confirms diverge (xfail); a word-granular op may not surface
# the bug via word 0 (word order is preserved under ev_mixed) and would xpass — drop those.
b("av_vmrghb", load_distinct(1)+[vx(2,1,1,12)] +grab(), "vmrghb v2,v1,v1: high-byte merge — ev_mixed element-order")
