...
# AltiVec merges vmrgh/l {b,h,w} FIXED 2026-06-04 — all on DISTINCT operands (vA=v1=00..0F,
# vB=v3=10..1F). Byte/halfword use ev_mixed-aware codegen (REV32.16B normalize -> ZIP -> REV32.16B
# back; ppc-jit.cpp emit_vmrg); word merges are plain ZIP.4S (word_element identity). All verified
# xpass; promoted from QUARANTINE to the scored gate.
...
# vpkuhum FIXED 2026-06-04 — pack 8+8 halfwords to low bytes; ev_mixed-normalized UZP2.16B
# (emit_vmrg). Distinct operands; verified xpass + boot-clean; promoted from QUARANTINE.
...
# Variable BYTE shifts FIXED 2026-06-06 (ppc-jit.cpp case 260/516/772). Were unmasked,
# signed, rounding NEON shifts; vsrb shifted the wrong direction. Now: amt&=7 (DUP #7 +
# AND) then truncating USHL/SSHL (+NEG for right). data=0x80..0x8F (high bit -> logical vs
# arith fill), amounts 0x00..0x0F (>=8 exercises the mod-8 mask). See docs/planning/ALTIVEC-SHIFT-ROTATE-BUGS.md.
...
TEST_ORDER+=(av_vsrab)
# Variable HALFWORD/WORD shifts FIXED 2026-06-06 (ppc-jit.cpp case 324/388/580/644/836/900).
# Same fix as the byte ops at .8H/.4S, validated against byte-ASYMMETRIC lvx operands (ev_mixed
# byte order covered). amounts exceed element width (hw base 0x10 -> >=16; word base 0x20 -> >=32)
# to exercise the mod-width mask. See docs/planning/ALTIVEC-SHIFT-ROTATE-BUGS.md.
...
TEST_ORDER+=(av_vsraw)
# Variable ROTATES FIXED 2026-06-06 (ppc-jit.cpp case 4/68/132). NEON has no vector rotate;
# synthesized as (x<<k)|(x>>>(w-k)), k=amt&(w-1). data=0x80.. so the wrapped bits show (a plain
# shift would drop them). See docs/planning/ALTIVEC-SHIFT-ROTATE-BUGS.md.
...
TEST_ORDER+=(av_vrlw)
# Saturating add/sub + signed averages FIXED 2026-06-06 (ppc-jit.cpp). Wrong NEON op
# (SABA/UABA/SMAXP) and/or swapped signedness -> SQADD/UQADD/SQSUB/UQSUB/SRHADD (capstone-
# verified). Boundary operands exercise saturation+signedness. ALTIVEC-SHIFT-ROTATE-BUGS.md / sweep.
...
# Even/odd unsigned BYTE multiplies FIXED 2026-06-04 (emit_vmul_byte: REV32.16B
# normalize -> UZP1/UZP2.16B even/odd select -> UMULL.8H widen -> REV32.8H). Distinct
# operands exercise both the selection AND the widening (even-lane products >255).
# Promoted from QUARANTINE — the ev_mixed quarantine lane is now EMPTY.
