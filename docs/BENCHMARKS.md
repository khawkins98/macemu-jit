...
| 2026-06-07 | **Fractal Carbon** | 1046 | 111M/s | 12.88 | JIT — ⚠️ runs its **scalar-FP path, NOT AltiVec** (hot block = 19-insn **FP**, ~14.5%; top-3 ≈ 43% of execution). SheepShaver isn't exposing AltiVec to the guest (#19), so this is an **FP** benchmark here. **Unchanged by P5b** (still 12.88) because its hot loop is **single-precision** FP (`fmuls`/`fmadds`/`fsubs`/`fnmsubs` + `fmr`, opcode 59) — P5b MVP only converted **double-precision** (opcode 63), which is what Speedometer Math uses (→ +16%). Converting the single-precision set + `fmr` is the next FP-RA increment and should light FC up. |
