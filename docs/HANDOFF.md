...
>   3. **Faithful-staging default** -- the default path (no `SS_M18_PARCEL_FILE`) still stages decompressed->garbage; make faithful the default by committing the ~2.4MB compressed `prcl` artifact OR extracting it at boot in `TrampolineStageParcels` (hook `trampoline_loader.cpp:296-305`; tool `SheepShaver/tools/extract-toolbox-parcels.py`).
