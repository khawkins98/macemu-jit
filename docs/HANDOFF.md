...
> - **OWED follow-ups (NONE rollback-worthy -- tree is clean, builds, test-jit=100, e2e PASS):**
...
>   2. ~~Review/gate B3's Cuda IFR/IER fix~~ -- **DONE (track-1c).** Gated-OFF byte-identical (newworld-only reachability).
>   3. ~~Faithful-staging default~~ -- **DONE (track-1a, `181db76b`).** `TrampolineStageParcels` defaults to boot-time `slice_compressed_prcl` from the prefs ROM; no committed blob; `SS_M18_PARCEL_FILE` is override-only. **Plus pre-S1 fail-loud hardening of these gated paths (`8cccd78c`): /mmu map overflow, direct-claim shim tripwire, setprop NULL-buf, prcl oversize, trace clamp -- see `docs/planning/newsheep/FINDINGS-pre-s1-bug-hunt.md` (incl. the verified-correct audit + the M1/M2 backstops S1 relies on).**
>   4. ~~CD-1 probe gate (S3)~~ -- **DONE (track-1b).** `[SPRG3+0x14]` confirmed ==`0x50314880` (R2). CD-3 still owed to S4 (NK EXT handler reads the interrupt-source block @`KDP-0x338`, ties to B3's Cuda).
