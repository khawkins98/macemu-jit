...
> - **▶ NEXT ACTION:** **execute the Stage 1 deep Task-0 recon** (NewWorld paged MMU). The Task-0 plan is OPENED + red-teamed (rev-2, GO-WITH-FIXES folded): `docs/superpowers/plans/2026-06-14-ss-m18-s1-paged-mmu-task0.md`. Recon job: pin Q-S1.1 (paravirtual byte-identical coexistence), Q-S1.2 (window via mach `vm_remap` on the **separate** NATMEM RAM/ROM reservations — **NOT** "MEM_BULK", which isn't compiled in this build; keep the walker a live fallback until 16K/4K host-page adequacy closes), Q-S1.3 (segment+context-table faithfulness); SPEC the MMU-oracle unit test (G1.a SPLIT — pure translation diff is unit-testable, "under-the-JIT" is owed to a new kpx_cpu harness or G1.e on a real boot). Output → `FINDINGS-s1-paged-mmu.md` with a GREEN-PASS/RESIDUE-PASS verdict. Program STATUS table: `…/2026-06-14-ss-m18-trampoline-lle-program.md`. Do NOT re-run the SS_M18 gating Task-0 or Discriminator-A (both done). Do NOT relitigate Route A.
