...
|---|---|---|---|---|
| 1 | **AltiVec Fractal Carbon 1.3** (Dauger Research) | **Purpose-built AltiVec** Mandelbrot; **falls back to scalar on non-G4 → built-in A/B oracle.** No license, no network, no source media. The cleanest possible vector torture test. **Already installed + boot-validated** (renders live; CarbonLib 1.6 baked into the workload disk). | [daugerresearch.com/fractals/altivecfractalcarbon.shtml](https://daugerresearch.com/fractals/altivecfractalcarbon.shtml) | Needs a window (not headless); OS 8.5+/CarbonLib 1.2+. **Zero-friction.** |
| 1b | **Power Fractal 1.4.1, Carbon CFM** (Dauger Research) | Fractal Carbon's heavier sibling — numerically-intensive parallel fractal that **self-reports GigaFlops**, giving an **absolute throughput metric with real-PowerPC-hardware anchors** (G4/450 ≈ 1.5 GFLOPS, DPG5/2GHz ≈ 13.5 GFLOPS single-machine). Uses AltiVec when present, **computes correctly on single-CPU / non-vector** → runs on our 1-core guest. So "our JIT achieves N GFLOPS vs a real G4/450's 1.5" becomes a concrete, citable number. The **Carbon CFM** build runs on **OS 8.5+ with CarbonLib 1.2+** (we have 1.6). | [daugerresearch.com/fractals/powerfractal.shtml](https://daugerresearch.com/fractals/powerfractal.shtml) (160 kB) | MPI/cluster features irrelevant single-node; just needs the single-machine compute. Same install path as Fractal Carbon (host-side onto the apps disk). |
