...
1. **Add a >8-live-GPR eviction vector.** ✅ **DONE** — `lmw_stmw_wide` (li
   ...
   `make test-jit` (238/238)**. CAVEAT: it only validates the JIT spill path in
...
1. **JIT codegen gate**: `make test-jit` (`SS_HARNESS_MODE=jit`) — runs every
   vector through the interpreter AND the JIT and diffs them. **238/238,
   ...
   interpreter determinism and proves nothing about the JIT — never cite a bare "238/238"
   without the mode.
