...
| **M6** | **PPC→68k handoff + shim triage** *(inherited Path A walls — HANDOFF §2.8 Phases 1–2, previously hidden inside "integration")*. **M6a rung 2 ✅ complete 2026-06-11** (plan: `docs/superpowers/plans/2026-06-11-m6a-rung2-mixedmode-switch.md`; results: `docs/planning/machine/M6A-ONGOING-ENTRY-DESIGN.md` Task T…Y sections): the DR cold-start dispatch runs (M6a Waves 1–2 — the old `rfi`-to-garbage crash is long resolved; jDR carries the boot) and the **68k→PPC Mixed Mode switch is COMPLETE in both directions, newworld profile DEFAULT** (`SS_NW_MM_SWITCH=0` opt-out; pool default-on + Hnfo-collision fix `735f775c`; forward switch = two KDP seeds `1c10d968`; table[0] cold/warm discriminator `3dd1550b`; `[KDP+0x65c]` world-flip closes the round trip `43d42b83` + the `[0x2810]` DEC fence; verify-and-leave + re-census `aaaa9a02`; acceptance + default flip `296c3661`). First complete MixedMode round trip: MPLibrary's TVector executes, 68k resumes at the completion-written PC, jNK 116M→4104, all gates green, opt-out byte-identical. **Next M6 milestone — the NK syscall surface**: MPLibrary's first kernel call dies at `sc` pc=0x500d638c, **unresolved vector 0xC00** (SRR0=0x500d6390 SRR1=0x00007072 lr=0x500cf108 r1=0x103ffb50; captured baseline comp=3672 jNK=4104 exc=0/1/0/0, CUDA quiet) — stop-rule trigger 2 captured-and-stopped — *and now ACTIVE as its own planned milestone (2026-06-11): plan `docs/superpowers/plans/2026-06-11-nk-syscall-surface.md` (rev 2, two red-team rounds folded); Task 0 recon COMPLETE (addendum in M3A-ENTRY-TABLE.md, `3e7b04ca`): entry pinned `syscall_entry=0x50314ac0` (primary copy, published at [KDP+0x390]), shim = exactly two SPR writes (SPRG1:=r1, SPRG2:=LR), first sc = selector 0x3f / result r3 / error r3≠0; the no-shim override boot already ran selector 0x3f end-to-end (r3=0) — implementation (Task A+) in flight.* | DR Emulator cold-start ECB/dispatch-table completion ✅ (Waves 1–2); MixedMode 68k→PPC switch ✅ (rung 2); NK syscall surface (vector 0xC00) — IN FLIGHT; `patch_68k` shim triage for the 9.0.1 ROM (28 in-range / 31 relocated-unverified / 25 absent — incremental, boot-path-first per the obstacle map). | **L (1–2+ wks, incremental)** |
