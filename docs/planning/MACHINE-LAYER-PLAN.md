...
| **M9** | ⏳ **VIA-IFR → guest-claimed 68k ticks — ACTIVE, stalled** (2026-06-12): infrastructure committed — OP_IRQ_NW frame-aware handler + `M68kRegisters.pc` writeback (`emul_op.cpp`, `main.h`, `sheepshaver_glue.cpp`); `via_nw901_int` ROM patch installs OP_IRQ_NW+rte at 0x5000ed08; harness 353/353. Boot stall with `SS_NW_VIA_IFR=1` **unresolved** — OP_IRQ_NW never fires; stall is upstream of the handler (with `SS_NW_VIA_IFR=1`: dec_expiries=5, fired=1 vs 7222/112 baseline). Two suspects: trampoline tp[25-26] vs ROM patch (not yet isolated). **Next step**: add `SS_NW_VIA_IFR_ROM_PATCH=1` gate for isolation, run two boots, identify root cause. Gate: `SS_NW_VIA_IFR`. | `SS_PROBE_68K=0x5000ed08` fires; dec_expiries recovers to ~2393 with `SS_NW_VIA_IFR=1`; Ticks guest-claimed; harness 353/353 throughout. | M |
