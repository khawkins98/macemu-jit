...
> **Current state (header budget = 5 lines; the full arc lives in `CHANGELOG.md` and the
> archived narrative at the bottom of this doc):** SheepShaver boots 8.6 to Finder, full
> native JIT (stable). Machine Layer: through **M7 interrupt injection SHIPPED default-ON**;
> **M8 slot-4 consumption CLOSED, shipped gated-off-green** (`SS_NW_IRQ_CONSUME`, flip
> prerequisites in its plan). **Active critical path: the VIA-IFR surface** (the 68k handler
> rte's source-less; resolve a4@0x5000ee9a first). Live frontier detail:
> `docs/AGENT-CONTEXT.md` "Current frontier" · history: §"Archived status narratives" below.
...
> **Progress:** spikes S1–S3 ✅ · M0 ✅ · M1 ✅ · M2 ✅ · **M3a ✅ 2026-06-10**: exc_core
> (OEA masks; ExcEnter/ExcRfi/ExcDeliverable; machine suite 9/9; 25 anti-vacuity checks); sc →
> CFLOW_TRAP; sc/rfi real semantics on newworld; EE-edge re-raise; DEC delivery hook (KDP shim,
> execute_depth gate, in-place poll); [NW-INT] deleted; HandleInterrupt fenced;
> cold-MSR EE=0 fix (0xf072→0x7072 at newworld trampoline); SCC Rx FIFO + SS_SCC_RX_INJECT.
> **First real PPC exception ever**: `[EXC] DEC delivered #1: restart=50429b40 srr1=0000f072
> msr=00001040 -> entry=50412b1c`. Both delivery directions verified (deliver when EE permits;
> defer exc=0/1/0 when not). **End-to-end demo**: SS_SCC_RX_INJECT CR at T+25s → JIT comp
...
> but cycles its Cuda probe sequence — loop-ender ROOT-CAUSED (`b3a947e4`, `36f048a0`): a
> deterministic ~80 ms reboot loop fired at MPLibrary's MixedMode FE01 service hitting an
> unprovisioned save-record pool + our own always-cold table[0] trampoline (self-inflicted
> scaffolding debt; Ticks-starvation hypothesis falsified;
> record: `docs/planning/machine/M6A-WAVE2-SHIM-RECON.md` "M3b Wave 1 acceptance").
> **M6a rung 2 ✅ 2026-06-11** (plan: `docs/superpowers/plans/2026-06-11-m6a-rung2-mixedmode-switch.md`;
> results: `docs/planning/machine/M6A-ONGOING-ENTRY-DESIGN.md`; CHANGELOG entry same date):
> the **68k→PPC Mixed Mode switch is complete in BOTH directions and is the newworld
> profile DEFAULT** (`SS_NW_MM_SWITCH=0` opt-out, pool `SS_NW_MM_POOL` default-on) —
> save-record pool + Hnfo-collision fix + occupancy map (`735f775c`), two-KDP-seed forward
> switch (`1c10d968`), table[0] cold/warm discriminator (`3dd1550b`), `[KDP+0x65c]`
...
> **M7 interrupt injection ✅ SHIPPED 2026-06-12**
> (`docs/superpowers/plans/2026-06-12-interrupt-injection.md`; arc `153c088b`…`81d60cc1`;
> evidence `INTERRUPT-INJECTION-RECON.md`): the **first host→guest interrupt delivered
> through the guest's own exception path** — host timer edge → once-per-assert-edge
> latch (`SS_NW_HOST_IRQ`, `b124556f`) → EXC_EXTERNAL → NK-published 0x50314880 →
> `[KDP+0x5b0]` fallback service body → **first guest IACK of the OpenPIC model ever**
> (B-2 level staging `68c8fc3e`, sign-off shape (i): host edge rides PIC input 0x3F) →
> the NK post's level test PASSES (watch 68fff070=80010001 class) env-on. Exactly-once
> per assert edge proven (edges=consumed, zero tripwires); Q-I6 fence narrowed for the
> published route (deferred_native 97→0). **THE CLUSTER FLIPPED** (`81d60cc1`):
> SS_NW_EE_RISER + SS_NW_DEC_PUBLISHED + SS_NW_HOST_IRQ newworld DEFAULT (explicit-"0"
> opt-outs; pre-flip checklist 4/4 `2a452166` incl. the ClearInterruptFlag lost-edge
> race fix); default boot delivers DEC #1–4 (2-SPR) + EXT #1 and still reaches the
> PROGRAM#5 park; all-OFF opt-out byte-identical to the pre-flip baseline. W2-4's
> reserved flip + absorbed items closed per its supersession table.
> **M8 slot-4 consumption round trip ✅ CLOSED 2026-06-12 — SHIPPED GATED-OFF-GREEN**
> (plan `2026-06-12-slot4-consumption.md`; commits `42ce3e0e`…`52b69cd7`; evidence
> INTERRUPT-INJECTION-RECON.md Task 0/B/C sections). The honest criteria table:
> world-switch sub-contract PASS (torn-ctx fork-(iii) root cause fixed — deferred
> EE-edge latch, the R-II10 shapes A/B/C all closed); round-trip legs 1–7 PASS env-on
> (post → Q-C3 staging → drain → slot-4 twi → 68k level-1 handler at 60 Hz);
> retirement (leg 8) RED — the via6522 IFR presents no 60 Hz source, the handler
> rte's source-less, the post re-traps; multi-edge structurally pre-WLSC-unreachable
> (gate flaw recorded); Ticks NOT guest-claimed (host keep-set census-proven);
> **default flip REFUSED** (deterministic SC#1=0x0d divergence on the flip-candidate
> config, 2/2 vs 0/19) — `SS_NW_IRQ_CONSUME` ships as a standalone 17th gate, default
> OFF, fold-into-cluster MANDATORY at its future flip (prerequisites: VIA-IFR lands +
> SC#1=0x0d explained-or-fixed).
> **▶ Current default-boot frontier: the PROGRAM#5 srr0=0x50324fec park (the NK idle
> nap loop) + live delivery chronology — unchanged by M8 (gated off).**
> **▶ ACTIVE critical path / named next task: the VIA-IFR surface** (device-model
> M-class — `dev_via6522` exists; present the 60 Hz source bit to the 68k level-1
> handler's IFR read. Task-0 recon questions, named by M8 Task C: resolve **a4 at
> 0x5000ee9a** — the MMIO address of the IFR the handler bit-tests — and which bit
> d6 indexes; the `$6e4` vector-chain dismissal; the SC#1=0x0d mechanism. Closing it
> retires leg 8 + unlocks the multi-edge invariant and the Ticks guest-claim.)
> **M7/M8 follow-on rows (owned, not blocking):**
> - **SS_NW_PIC default flip — HELD** (W2-3 stop-rule 3): criteria = real guest MPIC
>   init (or device-source acceptance) + the tripwire-counter per-source split
>   (Task C item 4 binding condition) + re-audit the stale SetInterruptFlag
>   caller-inventory/lock comment (Task-C review P3: ClearInterruptFlag now calls the
>   full edge path incl. pic_input_locked). Until then SS_NW_PIC=1 is the env-on TEST
>   cluster (carries the B-2 level staging — default boots deliver at level 0, R-II7).
>   **M8 consumption input (2026-06-12)**: the consumption rail downstream of the
>   level staging is now proven through the 68k handler (one of this flip's written
>   criteria inputs); still held — guest MPIC init + the per-source split + the
>   VIA-IFR retirement remain outstanding.
> - **Gate retirement UN-DEFERRED (flag only, 2026-06-12)**: re-score #3's coordinator
>   deferral ("no retirement churn while the consumption half is in flight") expires
>   with the M8 close-out — the six pre-M7 default-ON surfaces are again eligible
>   retirement candidates after a quiet release cycle (flagged, not scheduled).
> - **Legacy-KDP-shim retirement + M3A dual-mode-row cleanup** (deferred from Task C):
>   the opt-out `SS_NW_DEC_PUBLISHED=0` route is the KDP shim's last consumer; retire
>   both together (M3A-ENTRY-TABLE "Residue" row updated by `81d60cc1` — the legacy
>   row is historical post-flip; schedule the retirement when the opt-out window ends).
> - **polled-trampoline + SDL_PumpEvents decisions — NAMED-DEFERRED** (W2-4
>   supersession default: leave / keep+decouple): the ROM VIA/SCC pollers coexist with
>   delivery; untouched by the M7 chain; re-evaluate when the 68k consumption side is
>   live (slot-4 task or later).
> SDL_PumpEvents relocation + deliverability harness vector + nested-execute path
> completion remain Wave-2 backlog.
...
### Header narrative as of 2026-06-12 (the M0→M8 arc)

D3 Machine Layer: M0 + M1 + M2 + M3a + M3b Wave 1 + M6a rung 2 + NK syscall surface ✅ +
FE1F service surface ✅ + 68k PC-desync ✅ all newworld DEFAULT; M3b Wave 2: OpenPIC LANDED
(`b86449c9`), W2-3 EXT delivery shipped gated-off, **W2-4 steps 0–2 landed gated-off**
(`181efc02`/`10b1b3e8`: SS_NW_DEC_PUBLISHED + SS_NW_EE_RISER — **first DEC deliveries
ever**, 12.4M storm-scale, then `delivered_dec=3` live on the published route); 2026-06-12:
slide wall FIXED (`2ff7765f` tm_task guard), **SysError-12 wall CLEARED**
(`f808a7fb`/`adea99bc` SS_NW_TM_TRAPS default-ON), **P-M5 Execute68k SIGSEGV CLEARED**
(`34d3d441` [KDP+0x1074/78] staging + `3cb3b16e` DEFER_NATIVE wake-up), DEC cadence fixed
(`f31d475e` [KDP+0xf2c]); **M7 interrupt injection ✅ SHIPPED 2026-06-12**
(`2026-06-12-interrupt-injection.md`, commits `b124556f`…`81d60cc1`): first host→guest EXT
delivery through the guest's own exception path, first guest IACK of the OpenPIC model,
the NK post's level test passes env-on, **the riser/published/host-irq cluster flipped to
newworld DEFAULT** (`81d60cc1`; `SS_NW_PIC` stays HELD); default-boot frontier = the
PROGRAM#5 srr0=0x50324fec park + live delivery chronology (DEC #1–4 published + EXT #1);
**M8 slot-4 consumption round trip ✅ CLOSED 2026-06-12 — SHIPPED GATED-OFF-GREEN**
(`2026-06-12-slot4-consumption.md`, commits `42ce3e0e`…`52b69cd7`; `SS_NW_IRQ_CONSUME`
standalone 17th gate, default OFF): the R-II10 livelock shapes FIXED (fork-(iii) torn-ctx
root cause — our riser's mtmsr re-raise mid-tail — cured by the deferred EE-edge latch +
Q-C3 staging + 60 Hz backstop); round trip GREEN through leg 7 env-on (post → staging →
drain → slot-4 twi → 68k level-1 handler at 60 Hz); default flip REFUSED on honest
criteria (the deterministic SC#1=0x0d divergence + retirement RED at leg 8); **named next
task: the VIA-IFR surface** (device-model M-class — the 68k handler rte's source-less;
resolve a4@0x5000ee9a + the d6 bit index first); standing process/tooling: gate tiers
(MILESTONE-WORKFLOW §6) + parallel-boot slot protocol (`SheepShaver/tools/README-slots.md`)
+ the `docs/AGENT-CONTEXT.md` pack.
