...
|---|---|---|---|---|
| `mdec_dat` (in `patch_nanokernel()`) | 1642–1655 | Stops the nanokernel from programming the real decrementer (`li r31,0` + NOPs the `mtdec` path) — DEC never ticks under guest control. | RETIRE@M2 | M2 (virtual clock honors `mtspr`/`mfspr` DEC; absorbs `SS_SYNTH_DEC` + M1's minimal tick) |
...
| `scc_init_caller_dat` + `scc_init_dat` | 2106–2119 | Locates the SCC init routine via its caller at vector 0x1ac, then replaces it with `M68K_EMUL_OP_RESET` + RTS — the guest never initializes the SCC. | **RETIRE@M1** | M1 (SCC 8530 model is M1's core device; M1 DoD asserts the un-patched init's WR write sequence reaches the model — SPIKE-S3 §1.3 lists the required write set) |
