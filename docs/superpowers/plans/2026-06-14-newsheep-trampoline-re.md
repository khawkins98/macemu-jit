...
- No placeholder tokens. The few runtime addresses an executor must discover (the OF client-interface entry, the structure's QEMU-space address, the Trampoline's claimed run address) are explicitly *outputs* of named earlier steps (Task 2 Step 3, Task 3 Steps 2/4) — discovered by the procedure, not hardcoded. The Trampoline artifact + entry (`MacOS.elf`, `0x20f078`) and tbxi invocation are now pinned (empirically verified), not parameterized.
- **Red-team rev-2 folds:** producer reframe/Q0-F (BLOCKER), `tbxi dump -o` + `MacOS.elf` (BLOCKERs), Q0-B null hypothesis (BLOCKER), mechanism-level agreement gate (BLOCKER), Q0-A two-part falsifiable criterion + Route-A cost (B2), 4-cell truth table (S1), DoD-negative costed (S2), per-route feasibility check (S3), Python gdb-remote client + `-S` (tracer BLOCKERs), address-space discipline in Task 2/3, scope-guard invariant (N2). ✓
