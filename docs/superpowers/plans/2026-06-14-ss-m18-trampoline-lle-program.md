...
| **S1 — paged MMU** | **▶ IN PROGRESS — Task A DONE (adversary-fixed); de-risk DONE (2-stage adversary) → Task B (window) UN-PARKED conditionally** | **Task A complete** (`fc3ca256/2bf1526b/191fe67c`, test-jit=100). **De-risk verdict = WINDOW-SAFE-BY-MECHANISM, numeric coverage owed to G1.e** (`FINDINGS-s1-window-derisk.md` rev-2): adversary confirmed no PTE/`tlbie` path provably writes a JIT-covered EA + all 7 tlbie sites avoid BATs (BAT writes are mtspr-only = hooked wholesale), but cut the recon's DBAT over-claim → **the window's safety = one runtime predicate (RAM+ROM are DBAT-covered, not just IBAT), confirmed at G1.e**. **NEXT:** open the Task-B window-build milestone (plan→3-reviewer→**user-authorized** gated build — LAW-adjacent `main_unix.cpp` vm_remap + mtspr hooks; real `make e2e` A/B+soak required; G1.e tightened to verify DBAT coverage). OWED: external oracle cross-check; Vs/Vp-vs-MSR[PR]. `…ss-m18-s1-impl-paged-mmu.md` | — (unblocked) |
