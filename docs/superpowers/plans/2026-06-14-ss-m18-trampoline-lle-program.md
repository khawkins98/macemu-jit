...
| **S1 — paged MMU** | **▶ IN PROGRESS — Task A DONE; window plan red-teamed → DEFERRED, BUILD ORDER FLIPPED to SOFTMMU-FIRST** | **Task A complete** (`fc3ca256/2bf1526b/191fe67c`, test-jit=100, translation core `paged_mmu_translate()` built+adversary-fixed). De-risk + Task-B plan 3-reviewer round → **WRONG-BUILD-ORDER**: the window's `vm_remap VM_FLAGS_OVERWRITE`-on-live-NATMEM primitive is unproven + topology-mismatched (single reservation = store AND access window, unlike Dolphin's separate arena), and "softmmu-default-until-measured" is un-implementable while softmmu is unwritten. **NEXT = the SOFTMMU-FIRST milestone:** softmmu walker (reuse `paged_mmu_translate()` at the interp chokepoint `vm.hpp`, gated, lower-risk, no foundation surgery) → paged boot at ZERO foundation risk → HARVEST live `(SR/BAT/SDR1,EA)` map + DBAT descriptors as constants → THEN the window as a pure A/B-validatable optimization (deferred plan `…s1-taskB-window-build.md` rev-2, re-validate against harvested constants + an overwrite-under-concurrency spike). `…ss-m18-s1-impl-paged-mmu.md` | — (unblocked) |
