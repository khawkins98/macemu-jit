...
| **S1 — paged MMU** | **▶ ACTIVE MILESTONE (GO granted 2026-06-15) — fast-path spike = its Task-0; NOT yet started (held for provider conversation).** Mechanism + static constants DONE; live MMU co-lands with S3. **Banked toward it (de-risk, NOT "nearly done"):** B1 standalone Dolphin shadow-arena module (`dolphin_bat_arena.*`, 43-check unit-tested, inert behind `paged_mmu_translate`) + the harvested 139-row NK SR/BAT/SDR1 oracle (`nk-faithful-oracle.log`). **REALITY CHECK:** the JIT fast-path interception/perf spine is UNSTARTED — that spike is S1 Task-0. **Two hard DoD:** (1) benchmark window-vs-softmmu on realistic access AND steady-state remap freq (measure remap cost under the MTSRIN per-context-switch loop @`0xf1529c` — window may be a syscall storm; must be in evidence before the call); (2) co-design S1+S3 as ONE thread. | **Mechanism DONE** (`fc3ca256/2bf1526b/191fe67c`+fix, test-jit=100): `paged_mmu_translate()` translation core + oracle test. **Static NK-MMU constants DONE** (`FINDINGS-s1-mmu-constants.md`, `f77702d9`). **Verdict: S1 live MMU ⟺ S3 — there is no live `(SR/BAT/SDR1)` map until the real NK install runs (S3); do NOT ARM/VALIDATE a live MMU against today's forge (but DO build/de-risk NK-independent machinery — see below). Full reasoning: `MMU-NANOKERNEL-INSEPARABILITY.md` (canonical).** Both live-MMU plans DEFERRED (re-validate at S3 against NK-installed tables): `…2026-06-15-ss-m18-s1-taskB-window-build.md` (DEFERRED), `…2026-06-15-ss-m18-s1-softmmu-first.md` (DEFERRED). **S1-MECHANISM = the true (satisfied) S1→S3 prerequisite; S1-LIVE-MMU co-lands WITH S3 (not a deadlock).** **Pre-S3 NK-independent de-risk (named, advisory 2026-06-15):** the **Dolphin SHM-arena foundation feasibility spike** (standalone/synthetic/gated-off — prove SHM+aliased-RW-views+16KB-page+overwrite-under-concurrent-reader on macOS arm64; the single biggest S1-live platform risk; §3 de-fuse-before-S3) + the cheap `SS_PROBE_PC` forge-state pass. `…ss-m18-s1-impl-paged-mmu.md` | **S1-mechanism DONE; live MMU ⟂ S3** |
...
| S2b — loader + /mmu + handoff | **✅ S2b-impl DONE (gated-OFF) — the REAL Trampoline runs** | T1–T5 landed (`trampoline_loader.cpp` staged-asset/compressed-prcl loader [boot-time `slice_compressed_prcl`, no committed blob], launch STUB, OF-CI claim/translate/map backends + EXEC_NATIVE shim, forge-gate). Behind `SS_M18_TRAMPOLINE`; paravirtual byte-identical. **Faithful-staging is DEFAULT** (`181db76b`): stage the compressed `prcl`, never decompressed aperture bytes. **NK body now executes + programs its full live SR/BAT/SDR1 map** — next wall = S1 live paged MMU | — |
| S3 — two-supervisor reconciliation | **✅ S3-impl restructure DONE (gated-OFF)** — architecture = REPLACE (4-reviewer red-teamed) | T1–T4 landed behind `SS_M18_NK_SUPERVISOR` ∧ newworld: gate + EXT-shim (`exc_inject.*`, `ExcResolveNkExtVector` reads KDP+0x374 — **CD-1 = R2 CONFIRMED**) + synthetic-supervisor retire + forge retire; KEEP exc_core vectoring (PEM=LAW); revert-on-red=branch revert; paravirtual byte-identical. **SHM-arena window PROVEN** (`acd89dce`/`cd7a62b6`). Live MMU co-lands with S1 (window front-runner). **Owed to first S3 boot:** device-IRQ→NK-EXT seam (CD-3, S4), sc path, G3.a/c positive probe | S1-mechanism (DONE) + S2b (DONE) |
| S4 — disk IM-init → CGRP | **✅ recon DONE (READ-ONLY); impl blocked on S3** | Recon banked (`FINDINGS-s4-im-init-cgrp.md` + `FINDINGS-s4-boot-target.md`): **CGRP = "motherboard coherence group"**, built **two-phase** by the disk `krnl 0` (replacement NanoKernel inside the 9.2.x System file), skeleton algorithm disassembled; boot target characterized (APM + blessed System Folder + `boot 1`, no separate Enabler; genuine retail 9.2.1/9.2.2 ISOs in hand). **S4 integration target = the builtin-NK → disk-`krnl 0` handoff (Old KDP→new KDP).** Next = S4 Task-0 live trace (writer-PC + DT read-order) | S3 |
