...
| **S1 — paged MMU** | **▶ IN PROGRESS — Task A DONE+committed (fc3ca256/2bf1526b/191fe67c, gated, adversary-fixed); Task B (window) PARKED** | **Task A complete:** `high_bat[16]` dead-code INSURANCE + `paged_mmu.cpp` translation core + test (paged_mmu adversary found+fixed a real-mode/BAT ordering bug the self-oracle was blind to; test-jit score=100). **NEXT (de-risk recon, read-only):** pin which guest PA ranges the live PTE engine @0x50319af8 targets vs the BAT-covered RAM/ROM the JIT uses — if disjoint, the window is earnable by analysis (collapses the G1.e chicken-and-egg). Then either build Task B (window + tlbie interception) or fall to softmmu. OWED: external PearPC/QEMU oracle cross-check; Vs/Vp-vs-MSR[PR] gate. `…ss-m18-s1-impl-paged-mmu.md` | — (unblocked) |
