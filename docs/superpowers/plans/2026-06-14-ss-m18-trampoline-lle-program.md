...
| S3 — two-supervisor reconciliation | **✅ Task-0 DONE → GREEN-PASS, architecture = REPLACE** (4-reviewer red-teamed). ▶ **S3-IMPL needs USER AUTHORIZATION** (the most dangerous flip) | Recon DONE: `FINDINGS-s3-{two-supervisor,nk-ownership,ss-reusemap}.md`; plan `2026-06-15-ss-m18-s3-two-supervisor-task0.md` rev-2. **Architecture = REPLACE** (CO-OWN/yield-fully scored out — co-residency impossible; thin-shim = strictly-larger-surface of REPLACE). **SHM-arena window PROVEN** (single `acd89dce` + multi-entry `cd7a62b6`: atomic one-step form, concurrent-reader-safe, 16K-coarse fork). Live MMU co-lands here (window front-runner). **S3-impl = retire SS synthetic supervisor under `SS_M18_NK_SUPERVISOR` ∧ newworld; KEEP exc_core vectoring (PEM=LAW); revert-on-red=branch revert. Awaiting user go.** Owed to S3-impl boot: device-IRQ→NK-EXT seam, sc path, G3.a/c positive probe | S1-mechanism (DONE) + S2 |
